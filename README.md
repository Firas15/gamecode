...
Исходный код платформы распространяется под лицензией **Apache License 2.0** — см. файл [LICENSE](LICENSE).
Перечень сторонних компонентов и их лицензий — в файле [NOTICE](NOTICE).

Коротко, что это значит: код можно свободно использовать, изменять и распространять,
в том числе в коммерческих проектах. Требуется сохранять уведомление об авторстве
и указывать, какие файлы были изменены. Лицензия включает явное предоставление
патентных прав от участников проекта.
